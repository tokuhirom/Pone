...
static void pone_compile_node(pone_universe* universe, PVIPNode* node, const char* filename, bool compile_only) {
    ...
    int retval = system("clang -D_POSIX_C_SOURCE=200809L -rdynamic -DPONE_DYNAMIC -fPIC -shared -lstdc++ -I3rd/rockre/include/ -Iinclude/ -I src/ -g -lm -std=c99 -o pone_generated.so pone_generated.c -L. -lpone 3rd/rockre/librockre.a");
    if (retval != 0) {
