...
static pone_val* meth_int_say(pone_world* world, pone_val* self, int n, va_list args) {
    assert(n==0);
...
void pone_int_init(pone_universe* universe) {
    ...
    pone_add_method_c(universe, universe->class_int, "say", strlen("say"), meth_int_say);
}
